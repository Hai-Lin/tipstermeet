...
User.tips = function (username, cb) {
  ...
  var url = this.client.baseUrl + 'user/' + username + '/tips?limit=80';
  this.client.get(url, cb);
};
...
User.following = function (username, offset, cb) {
  if (!username) {
    throw new Error('Must supply username to user function');
  }
var url = this.client.baseUrl + 'user/'+ username + '/following?limit=500&offset=' + offset;
  this.client.get(url, cb);
};
